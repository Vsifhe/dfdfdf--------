...
using namespace std;
abonent_pay station(5); 
abonent *offstation = new abonent(0);
            ...
            case 1:
                {
                    ...
                    cin>>sb->name; // строка которая является полем объекта по указателю sb. указатель используется чтобы не создавать новый объект, а изменять старый
...
void showSubscriber(){
    ...
    abonent* tmp;
    for( auto number: sb->get_recent_calls()){
        station.fill_subscriber(number, &tmp);
        cout<<tmp->get_description(number, false )<<endl;
    } 
	getch();
}
...
void makeCall(){
    ...
    cin>>number2;
    if(station.fill_subscriber(number2, &sb2)!=0){
        cout<<"Абонент с таким номером не найден"<<endl;
        return;
    } 
    int duration = 0;
	...
	switch(num){
            case 1:
                {
                    abonent *poly=offstation;
					poly->make_call(number, number2, duration);
                    cout<<"Успех!";
					break;
                }
            case 2:
                {   
                    poly=(abonent*)&station;
					int result = poly->make_call(number, number2, duration);
				    cout<<"Результат: ";
    switch(result){
        case 0: 
            {
